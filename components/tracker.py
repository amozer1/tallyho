...
def render_tracker(df):
    ...
    # =========================
    # CLEAN STATIC ALERT BOX (NO FLASHING)
    # =========================
    st.markdown(
        f"""
        <div style="
            ...
            max-width: 520px;
        ">
            <div style="font-size:14px; font-weight:700; color:#ff4d4d;">
                ⚠ Outstanding > 7 Days
            </div>

            <div style="font-size:18px; font-weight:800; color:white; margin-top:4px;">
                {overdue_total} ({overdue_total_pct}%)
            </div>

            <div style="margin-top:8px; font-size:13px; color:white;">
                🔵 TQ: <b>{overdue_tq} ({overdue_tq_pct}%)</b><br>
                🟢 RFI: <b>{overdue_rfi} ({overdue_rfi_pct}%)</b>
            </div>
        </div>
        """,
